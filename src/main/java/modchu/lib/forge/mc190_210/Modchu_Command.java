...
public class Modchu_Command implements ICommand {
	...
	public void execute(MinecraftServer server, ICommandSender iCommandSender, String[] astring) {
		if (master != null) master.execute(server, iCommandSender, astring);
	}

	@Override
	public boolean checkPermission(MinecraftServer server, ICommandSender iCommandSender) {
		return master != null ? master.checkPermission(server, iCommandSender) : false;
	}

	@Override
	public List<String> getTabCompletionOptions(MinecraftServer server, ICommandSender iCommandSender, String[] astring, BlockPos pos) {
		return master != null ? master.getTabCompletionOptions(server, iCommandSender, astring, pos) : null;
	}
