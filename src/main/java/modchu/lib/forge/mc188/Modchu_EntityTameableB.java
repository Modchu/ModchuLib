...
public class Modchu_EntityTameableB extends EntityTameable implements Modchu_IEntityTameableB {
	...
	public void onUpdate() {
		if (!worldObj.isRemote) {
			...
			if (ticksExisted % 20 == 0) {
				...
				//Modchu_Debug.lDebug("Modchu_EntityTameableB onUpdate combatTracker="+combatTracker);
				combatTracker.reset();
				ticksExisted++;
			}
		}
	...
	public void onDeath(DamageSource damageSource) {
		//CombatTracker combatTracker = getCombatTracker();
		//Modchu_Debug.lDebug("Modchu_EntityTameableB onDeath combatTracker="+combatTracker);
	...
	public void superOnDeath(Object damageSource) {
		if (!worldObj.isRemote && worldObj.getGameRules().getGameRuleBooleanValue("showDeathMessages") && hasCustomName() && getOwner() instanceof EntityPlayerMP) {
		...
		if (!worldObj.isRemote) {
			...
			if (canDropLoot() && worldObj.getGameRules().getGameRuleBooleanValue("doMobLoot")) {
	...
	public void superDamageEntity(Object damageSource, float par2) {
		if (!isEntityInvulnerable((DamageSource) damageSource)) {
			...
			if (par2 != 0.0F) {
				float f2 = getHealth();
				setHealth(f2 - par2);
				getCombatTracker2().trackDamage((DamageSource) damageSource, f2, par2);
...
*/
	...
	@Override
	public void func_181015_d(BlockPos bockPos) {
		if (master != null) master.setInPortal(bockPos);
		else super.func_181015_d(bockPos);
	}

	@Override
	public void superSetInPortal(Object bockPos) {
		super.func_181015_d((BlockPos) bockPos);
	}
