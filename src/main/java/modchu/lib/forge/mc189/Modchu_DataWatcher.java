...
public class Modchu_DataWatcher extends DataWatcher implements Modchu_IDataWatcher {
	...
	public int getEntityDataManagerEntriesCount() {
		...
		//Modchu_Debug.mDebug("Modchu_DataWatcher getEntityDataManagerEntriesCount watchedObjects="+watchedObjects);
		int i = 0;
	...
	public <T> void addObject(int id, T object) {
		if (master != null) master.addObject(id, object);
		else super.addObject(id, object);
	}
	...
	@Override
	public <T> void updateObject(int id, T t) {
		if (master != null) master.updateObject(id, t);
		else super.updateObject(id, t);
	}
