...
public class Modchu_RendererLivingEntity extends RendererLivingEntity implements Modchu_IRendererLivingEntity {
	...
	public Modchu_RendererLivingEntity(Class masterClass, Modchu_ModelBase model, float f) {
		super((RenderManager) Modchu_AS.get(Modchu_AS.renderManagerInstance), model, f);
		Object instance = null;
		try {
			instance = masterClass != null ? Modchu_Reflect.newInstance(masterClass, new Class[]{ Modchu_IRendererLivingEntity.class, Object[].class }, new Object[]{ this, new Object[]{ model, f } }) : null;
		} catch(Error e1) {
			e1.printStackTrace();
		} catch(Exception e) {
			e.printStackTrace();
