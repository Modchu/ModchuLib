...
//		Modchu_Debug.tDebug("getTextureBoxServer: %d / %d", pIndex, textureServer.size());
	...
	public boolean loadTextures() {
		...
		Modchu_Debug.tDebug("loadTexturePacks. 1 searchPrefix="+searchPrefix);
		for (String[] lss : searchPrefix) {
			Modchu_Debug.tDebug("loadTexturePacks getTexture[%s:%s].", lss[0], lss[1]);
			// mods
			for (File lf : Modchu_FileManager.getFileList(lss[0])) {
				Modchu_Debug.tDebug("loadTexturePacks. 2 lf="+lf);
				for (String[] lst : searchPrefix) {
					boolean lflag;
					if (lf.isDirectory()) {
						// ディレクトリ
						Modchu_Debug.tDebug("loadTexturePacks. 3 addTexturesDir");
						lflag = addTexturesDir(lf, lst);
					} else {
						// zip
						Modchu_Debug.tDebug("loadTexturePacks. 3 addTexturesZip");
						lflag = addTexturesZip(lf, lst);
					}
					Modchu_Debug.tDebug("loadTexturePacks 4 getTexture end. %s-%s.", lf.getName(), lflag ? "done" : "fail");
				}
			}
		}
...
*/
	...
	public void addModelClass(String fname, String[] pSearch) {
		...
		if (lfindprefix > -1
			...
			String pn = fname.substring(pSearch[2].length() + lfindprefix);
