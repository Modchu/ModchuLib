...
public class Modchu_EntityTameable extends EntityTameable implements Modchu_IEntityTameable {
	...
	public static ConcurrentHashMap<String, UUID> entityUniqueIDMap;
	protected UUID entityUniqueID;
	public int maxHealth;
	private boolean initFlag;
	private boolean entityInitFlag;

	public Modchu_EntityTameable(World world) {
		super(world);
		Modchu_Debug.mDebug("Modchu_EntityTameable init World start. isRemote="+(worldObj != null ? worldObj.isRemote : "worldObj null"));
		ignoreFrustumCheck = true;
		initFlag = false;
		setEntityHealth(1);
		init((HashMap)null);
		Modchu_Debug.mDebug("Modchu_EntityTameable init World end.");
	}
	...
	public Modchu_EntityTameable(HashMap<String, Object> map) {
		...
		setEntityHealth(1);
		init(map);
		Modchu_Debug.mDebug("Modchu_EntityTameable init HashMap end.");
	}
	...
	public Object getMaster() {
		//Modchu_Debug.mDebug("Modchu_EntityTameable getMaster master="+master);
		if (master != null) ;else {
			Modchu_Debug.mDebug("Modchu_EntityTameable getMaster entityName="+entityName);
			init(entityName);
		}
		//Modchu_Debug.mDebug("Modchu_EntityTameable getMaster return master="+master);
		return master;
	}

	protected void init(String s) {
		Modchu_Debug.mDebug("Modchu_EntityTameable init s="+s);
		if (s != null
				&& !s.isEmpty()); else return;
		Class c = Modchu_Reflect.loadClass(s);
		Modchu_Debug.mDebug("Modchu_EntityTameable init c="+c);
		if (c != null); else return;
		HashMap<String, Object> map = Modchu_Main.getNewModchuCharacteristicMap(c);
		init(map);
		Modchu_Debug.mDebug("Modchu_EntityTameable init String end.");
	}

	protected void init(HashMap<String, Object> map) {
		boolean isRemote = Modchu_AS.getBoolean(Modchu_AS.worldIsRemote, this);
		Modchu_Debug.mDebug("Modchu_EntityTameable init isRemote="+isRemote+" map="+map);
		if (posX == 0
				&& posY== 0
				&& posZ == 0) {
			Modchu_Debug.mDebug("Modchu_EntityTameable init isRemote="+isRemote+" initFlag="+initFlag+" pos0 return");
			return;
		}
		Modchu_Debug.mDebug("Modchu_EntityTameable init isRemote="+isRemote+" initFlag");
		if (map != null
				&& !map.isEmpty()); else {
			Modchu_Debug.mDebug("Modchu_EntityTameable init isRemote="+isRemote+" map == null.");
			Class c = Modchu_Main.getSpownEntityClass(worldObj, posX, posY, posZ);
			map = new HashMap();
			map.put("Class", c);
			map.put("Object", worldObj);
			Modchu_Debug.mDebug("Modchu_EntityTameable isRemote="+isRemote+" init c="+c+" worldObj="+worldObj);
		}
		if (map != null
				&& !map.isEmpty()); else {
			Modchu_Debug.mDebug("Modchu_EntityTameable init 2 isRemote="+isRemote+" map == null !!");
			return;
		}
		Modchu_Debug.mDebug("Modchu_EntityTameable init 3 isRemote="+isRemote);
		map.put("base", this);
		...
		String s0 = new StringBuilder(Modchu_AS.getBoolean(Modchu_AS.worldIsRemote, this) ? "1" : "0").append(uuid).toString();
		if (entityUniqueIDMap != null); else entityUniqueIDMap = new ConcurrentHashMap();
		if (s0 != null
				&& entityUniqueIDMap.containsKey(s0)) {
			Modchu_Debug.mDebug("Modchu_EntityTameable init isRemote="+isRemote+" entityUniqueIDMap.containsKey isDead s0="+s0);
			Modchu_AS.set(Modchu_AS.entityLivingBaseSetHealth, this, 0.0F);
			deathTime = 20;
			setDead();
			return;
		}
		if (s0 != null) entityUniqueIDMap.put(s0, uuid);
		initFlag = true;
		Modchu_Debug.mDebug("Modchu_EntityTameable init isRemote="+isRemote+" entityUniqueID="+entityUniqueID);
		Modchu_Debug.mDebug("Modchu_EntityTameable init masterEntity="+master);
	}

	public static void worldEventLoad(Object event) {
		Modchu_Debug.mDebug("Modchu_EntityTameable worldEventLoad");
		if (entityUniqueIDMap != null) {
			entityUniqueIDMap.clear();
		}
		Modchu_Debug.mDebug("Modchu_EntityTameable worldEventLoad entityUniqueIDMap="+entityUniqueIDMap);
	}
	...
	public void writeEntityToNBT(NBTTagCompound nBTTagCompound) {
		if (entityName != null
			...
			try {
				...
				Modchu_Debug.mDebug("Modchu_EntityTameable writeEntityToNBT uniqueID="+uniqueID);
				...
				if (master != null); else init(entityName);
				Modchu_Debug.mDebug("Modchu_EntityTameable writeEntityToNBT master="+master);
	...
	public void onUpdate() {
		...
		boolean isRemote = Modchu_AS.getBoolean(Modchu_AS.worldIsRemote, this);
		//if (isRemote)
			//Modchu_Debug.mDebug("Modchu_EntityTameable onUpdate isRemote="+isRemote+" initFlag="+initFlag);
	...
	protected void onDeathUpdate() {
		//boolean isRemote = Modchu_AS.getBoolean(Modchu_AS.worldIsRemote, this);
		//Modchu_Debug.mDebug("Modchu_EntityTameable onDeathUpdate isRemote="+isRemote+" initFlag="+initFlag+" deathTime="+deathTime);
	...
	public UUID getUniqueID() {
		if (master != null) return master.getUniqueID();
		if (entityUniqueID != null); else entityUniqueID = UUID.randomUUID();
	...
	public void setUniqueId(UUID uniqueIdIn) {
		...
		else entityUniqueID = uniqueIdIn;
	}
	...
	public void superSetDropChance(int par1, float par2) {
	}
	...
	public void superSetHealth(Object par1) {
		super.setEntityHealth(Modchu_CastHelper.Int(par1, 0, false));
	}
	...
	public boolean attackEntityFrom(DamageSource damageSource, int par2) {
		Modchu_Debug.mDebug("Modchu_EntityTameable attackEntityFrom damageSource="+damageSource+" par2="+par2);
	...
	public void superSetMaxHealth(Object floatOrInt) {
		maxHealth = Modchu_CastHelper.Int(floatOrInt, 0, false);
	}
