...
public class ModelPlayerFormLittleMaid_Yukkuri extends ModelPlayerFormLittleMaid
{
    ...
    public ModelPlayerFormLittleMaid_Yukkuri()
    {
        this(0.0F);
    }

    public ModelPlayerFormLittleMaid_Yukkuri(float f)
    {
        this(f, 0.0F);
    }

    public ModelPlayerFormLittleMaid_Yukkuri(float f, float f1)
    {
		// 132deletesuper(f, f1);
//-@-132
		this(f, f1 , 64, 32);
	...
	public ModelPlayerFormLittleMaid_Yukkuri(float f, float f1, int i, int j) {
		super(f, f1, i, j);
//@-@132
	}
...
//ë´äpìxÉ}ÉCÉiÉXÇ≈ÉWÉÉÉìÉv
    	...
    	Skirt.setRotationPoint(0.0F, 0.0F, 0.0F);
        //bipedRightArm.rotationPointY = bipedLeftArm.rotationPointY = bipedHead.rotationPointY - 3F;
...
//â°îØ ï‡Ç¢ÇΩÇ∆Ç´äOÇ…çLÇ™ÇÈÇÊÇ§Ç…ÅB
...
//å„ÇÎîØ ï‡Ç¢ÇΩÇ∆Ç´äOÇ…çLÇ™ÇÈÇÊÇ§Ç…
...
//h2 óÉÅAé©ìÆìÆÇ´ + à⁄ìÆìÆÇ´ ãtë§
        ...
        {
        	bipedHead.rotationPointY = bipedRightLeg.rotateAngleX < 0.0F ? 20F : 20F - bipedRightLeg.rotateAngleX * 8F;
            //bipedBody.rotationPointY = bipedHead.rotationPointY;
        }
        ...
        if(getaimedBow())
        {
			...
			float f7 = MathHelper.sin((1.0F - (1.0F - getOnGround())
